...
*   **Constructor `GeneticAlgorithm(int population_size, double mutation_rate, double crossover_rate, int num_generations, const NeuroNet::NeuroNet& template_network)`:**
    *   Initializes the GA with essential parameters and a default generation limit.
    *   The four-argument overload `GeneticAlgorithm(population_size, mutation_rate, crossover_rate, template_network)` lets each run supply its generation limit. Its default limit is zero.
