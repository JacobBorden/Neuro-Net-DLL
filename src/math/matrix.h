...
namespace Matrix
{
	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		for (size_t i = 0; i < m_Rows; i++) {
            ...
            // Optimization: Changed the loop order from i-k-j to i-j-k to drastically improve cache hit rate
            // by iterating sequentially in memory (row-major memory access for b and c).
