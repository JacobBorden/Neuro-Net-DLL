...
namespace Matrix
{
	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		for (size_t i = 0; i < m_Rows; i++) {
            ...
            // Optimization: Changed to i-j-k loop order to improve cache locality.
            // By traversing the inner loop sequentially over memory blocks, cache
            // misses are minimized resulting in significantly faster multiplications.
			for (size_t j = 0; j < m_Cols; j++) {
                T a_ij = m_Data[i][j];
                for (size_t k = 0; k < b.m_Cols; k++) {
