	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		for (size_t i = 0; i < m_Rows; i++) {
			...
			for (size_t j = 0; j < m_Cols; j++) { // Iterate over columns of a / rows of b
				T a_ij = m_Data[i][j];
				// i-j-k loop interchange for matrix multiplication optimization.
				// This loop order maximizes spatial locality by sequentially accessing memory in b.m_Data,
				// rather than jumping across columns, resulting in fewer cache misses.
