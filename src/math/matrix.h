...
namespace Matrix
{
	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		// Bolt performance optimization: Changed loop order from i-k-j to i-j-k.
		// The i-j-k loop interchange ensures sequential memory access in the innermost loop (k),
		// significantly reducing cache misses and improving performance for row-major matrices.
