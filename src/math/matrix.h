...
namespace Matrix
{
	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		// Parallelize the outermost loop using OpenMP.
		// Optimized to ikj loop order for cache friendliness (sequential memory access in inner loop).
#ifdef _OPENMP
		#pragma omp parallel for
#endif
		for (size_t i = 0; i < m_Rows; i++) {
			for (size_t k = 0; k < b.m_Cols; k++) {
				c.m_Data[i][k] = T(0); // Explicitly initialize the matrix element to zero
			}
			for (size_t j = 0; j < m_Cols; j++) {
				T a_ij = m_Data[i][j];
				for (size_t k = 0; k < b.m_Cols; k++) {
