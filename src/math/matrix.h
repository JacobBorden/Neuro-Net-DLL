...
namespace Matrix
{
	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		// ⚡ Bolt Optimization: Using i-j-k loop interchange instead of naive i-k-j.
		// This drastically improves CPU cache locality by ensuring sequential memory access
		// in row-major format, which significantly reduces cache misses and speeds up execution.
