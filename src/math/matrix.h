...
namespace Matrix
{
	...
	Matrix<T> operator*(const Matrix<T> &b) const
	{
		...
		for (size_t i = 0; i < m_Rows; i++) {
			// Initialize row to 0 to avoid reliance on std::make_unique value-initialization
			for (size_t k = 0; k < b.m_Cols; k++) {
				c.m_Data[i][k] = T(0);
			}
			// ⚡ Bolt: Use i-j-k loop interchange for cache-friendly sequential memory access.
			// This avoids jumping between rows of 'b' in the inner loop, drastically reducing cache misses.
			for (size_t j = 0; j < m_Cols; j++) {
				T a_val = m_Data[i][j];
				for (size_t k = 0; k < b.m_Cols; k++) {
