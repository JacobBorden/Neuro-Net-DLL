...
void NeuroNet::NeuroNetLayer::ApplyReLU(Matrix::Matrix<float>& io_matrix) {
    for (int i = 0; i < io_matrix.rows(); ++i) {
        for (int j = 0; j < io_matrix.cols(); ++j) {
            io_matrix[i][j] = std::max(0.0f, io_matrix[i][j]);
        }
    }
}

void NeuroNet::NeuroNetLayer::ApplyLeakyReLU(Matrix::Matrix<float>& io_matrix) {
    const float alpha = 0.01f;
    for (int i = 0; i < io_matrix.rows(); ++i) {
        for (int j = 0; j < io_matrix.cols(); ++j) {
            if (io_matrix[i][j] < 0) {
                io_matrix[i][j] = alpha * io_matrix[i][j];
            }
        }
    }
}

void NeuroNet::NeuroNetLayer::ApplyELU(Matrix::Matrix<float>& io_matrix) {
    const float alpha = 1.0f;
    for (int i = 0; i < io_matrix.rows(); ++i) {
        for (int j = 0; j < io_matrix.cols(); ++j) {
            if (io_matrix[i][j] < 0) {
                io_matrix[i][j] = alpha * (std::exp(io_matrix[i][j]) - 1.0f);
            }
        }
    }
}
...
void NeuroNet::NeuroNetLayer::ApplySoftmax(Matrix::Matrix<float>& io_matrix) {
    float sum_exp = 0.0f;
    ...
    for (int j = 0; j < io_matrix.cols(); ++j) {
        io_matrix[0][j] = std::exp(io_matrix[0][j]);
        sum_exp += io_matrix[0][j];
    }

    if (sum_exp != 0.0f) {
        for (int j = 0; j < io_matrix.cols(); ++j) {
            io_matrix[0][j] /= sum_exp;
        }
    }
}
