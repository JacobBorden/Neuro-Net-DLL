...
NeuroNet::NeuroNet run_xor_evolution() {
...
void test_xor_model(NeuroNet::NeuroNet& best_model) {
    if (best_model.getLayerCount() == 0) return;
...
void test_xor_model_json(NeuroNet::NeuroNet& best_model) {
    if (best_model.getLayerCount() == 0) return;
    ...
    for (size_t i = 0; i < test_inputs.size(); ++i) {
        ...
        try {
            ...
            if (output_json_val.type == JsonValueType::Object) {
                ...
                output_json_val.GetObject().clear();
            }
...
void demonstrate_string_input_vocabulary(NeuroNet::NeuroNet& best_model) {
    if (best_model.getLayerCount() == 0) return;
    ...
    if (vocab_loaded) {
        ...
        const auto& loaded_vocab_obj = best_model.getVocabulary();
...
int main() {
    NeuroNet::NeuroNet best_model = run_xor_evolution();

    test_xor_model(best_model);
    test_xor_model_json(best_model);
    demonstrate_string_input_vocabulary(best_model);
