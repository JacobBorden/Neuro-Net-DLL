...
NeuroNet::NeuroNet train_xor_model() {
    ...
    std::cout << "  Input Size: " << template_network.getLayer(0).WeightCount() / template_network.getLayer(0).LayerSize() << std::endl;
    ...
    if (best_model.getLayerCount() > 0) {
        ...
        std::cout << "Best model has no layers, cannot save." << std::endl;
    }

    return best_model;
}

void test_model_on_xor(NeuroNet::NeuroNet& best_model) {
...
void demonstrate_string_input_and_vocabulary(NeuroNet::NeuroNet& best_model) {
    ...
    if (vocab_loaded) {
        ...
        // Removed inline comments to prevent JSON parsing issues.
        const std::string string_input_json = R"({
            "input_batch": [
                "hello world neuronet",
                "an unknown example",
        ...
        try {
            if (best_model.SetStringsInput(string_input_json)) {
                ...
                // 5. Get and display output (as JSON)
...
int main() {
    NeuroNet::NeuroNet best_model = train_xor_model();

    if (best_model.getLayerCount() > 0) {
        test_model_on_xor(best_model);
        test_model_with_json(best_model);
        demonstrate_string_input_and_vocabulary(best_model);
    } else {
        std::cout << "Best model has no layers, cannot test." << std::endl;
    }
