...
void test_model_on_xor_inputs(NeuroNet::NeuroNet& best_model) {
...
void test_model_with_json_io(NeuroNet::NeuroNet& best_model) {
...
int main() {
    ...
    if (best_model.getLayerCount() > 0) {
        ...
        test_model_on_xor_inputs(best_model);

        // Test with JSON input/output
        test_model_with_json_io(best_model);
