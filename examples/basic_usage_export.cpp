...
void test_best_model_on_xor(NeuroNet::NeuroNet& best_model) {
    ...
    for (size_t i = 0; i < test_inputs.size(); ++i) {
        ...
        for(size_t j = 0; j < test_inputs[i].size(); ++j) {
            input_matrix[0][j] = test_inputs[i][j];
        }
...
void test_best_model_with_json(NeuroNet::NeuroNet& best_model) {
...
void run_genetic_algorithm_xor_example() {
    ...
    if (best_model.getLayerCount() > 0) {
        ...
        test_best_model_with_json(best_model);
...
int main() {
    // Run the XOR genetic algorithm example
