...
The library supports exporting detailed training progress and model architectures.

### 1. Overview
- **`training_metrics.json`**: This file is serialized using the `nlohmann::json` library. It captures:
    - Overall statistics for a training run (start/end times, total generations).
    - Per-generation metrics (average fitness, best fitness).
    - The best model's architecture, embedded as an *escaped JSON string* which itself is in the custom library format.
- **Model Files (e.g., `best_model_custom_format.json`)**: These files are generated by `NeuroNet::save_model()` and use the library's custom JSON format. They are primarily intended for use with `NeuroNet::load_model()`.
...
### 3. `training_metrics.json` Format
This file is structured using `nlohmann::json` for the overall metrics. The best model's architecture is embedded as a string.
...
```json
{
    ...
    "best_model_architecture_params": {
        "model_custom_json_string": "{\"input_size\":2.0,\"layer_count\":2.0,\"layers\":[{\"input_size\":2.0,\"layer_size\":3.0,\"activation_function\":\"ReLU\",\"weights\":{\"rows\":2.0,\"cols\":3.0,\"data\":[...]},\"biases\":{\"rows\":1.0,\"cols\":3.0,\"data\":[...]}},{/*...next layer...*/}]}",
        "overall_best_fitness": 0.95
    }
}
```
- `start_time`/`end_time`: Timestamps for the training run.
- `total_generations`: The number of generations configured for the run.
- `generation_data`: An array where each object represents metrics for one generation.
    - `loss`/`accuracy`: Placeholders; default to `null` (from `NaN`) if not explicitly computed.
- `best_model_architecture_params`:
    - `model_custom_json_string`: An **escaped JSON string** representing the best model's architecture, generated by the library's custom `NeuroNet::to_custom_json_string()` method.
    - `overall_best_fitness`: The fitness score of this best model.
...
You can use Python to load `training_metrics.json`, parse the embedded model string, and visualize training progress:
...
# Load the training metrics (this file is nlohmann::json format)
with open("training_metrics.json", "r") as f:
    metrics_data = json.load(f)

# Extract and parse the embedded custom JSON model string
# The string from metrics_data is already unescaped by the initial json.load()
best_model_json_string = metrics_data["best_model_architecture_params"]["model_custom_json_string"]

# Now, parse this string, which is in the custom JSON format
# Standard json.loads should work if the custom format is valid JSON.
try:
    best_model_structure = json.loads(best_model_json_string)
    print("Best Model Structure (from custom JSON string embedded in metrics):")
    print(json.dumps(best_model_structure, indent=4))
except json.JSONDecodeError as e:
    print(f"Error decoding embedded custom JSON string: {e}")
    print("Offending string:", best_model_json_string)

print(f"Overall best fitness: {metrics_data['best_model_architecture_params']['overall_best_fitness']}")

# Convert generation data to a Pandas DataFrame for plotting
df_generations = pd.DataFrame(metrics_data["generation_data"])
...
if not df_generations.empty:
    ...
    print("No generation data found in metrics file to plot.")

```
Ensure you have `matplotlib` and `pandas` installed (`pip install matplotlib pandas`).

#### Direct Model Files (Custom Format)
Files saved by `NeuroNet::save_model()` (e.g., `best_model_custom_format.json`) can also be loaded and parsed in Python using `json.load()` or `json.loads()`.

```python
# Example for loading a model file directly (e.g., best_model_custom_format.json)
# Note: The custom library's ToString() method should produce valid JSON
# for this to work seamlessly with standard Python JSON parsers.
# Issues like non-standard handling of trailing commas or quoting could arise
# if the custom output is not strictly compliant.
# with open("best_model_custom_format.json", "r") as f:
#     try:
#         direct_model_data = json.load(f)
...
```
Parsing the custom JSON format might require some care if its output is not 100% compliant with standard JSON parsers (e.g., regarding strictness on trailing commas, number formats). However, the library's custom parser is designed to handle its own output for `NeuroNet::load_model()`.
